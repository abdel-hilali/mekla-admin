...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/livreurs/desaffecter-commandes`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Failed to unassign commandes" }, 
    ...
    console.error("Error unassigning commandes:", error);
    return NextResponse.json(
